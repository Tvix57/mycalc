...
TEST(calc_credit_test, test1_c) {
  ...
  double summ = 2109.96;
  ASSERT_NEAR(month_pay, test.get_month_pay(), 1e-6);
  ASSERT_NEAR(overpay, test.get_overpay(), 1e-6);
  ASSERT_NEAR(summ, test.get_allpayed(), 1e-6);
}
...
TEST(calc_credit_test, test3_c) {
  ...
  double summ = 2108.33;
  ASSERT_NEAR(month_pay_1, test.get_month_pay(), 1e-6);
  ASSERT_NEAR(overpay, test.get_overpay(), 1e-6);
  ASSERT_NEAR(summ, test.get_allpayed(), 1e-6);
  ASSERT_NEAR(month_pay_2, test.get_final_pay(), 1e-6);
}
